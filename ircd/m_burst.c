...
int ms_burst(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  struct Ban *lp, **lp_p;
  struct Except *ep, **ep_p;
  unsigned int parse_flags = (MODE_PARSE_FORCE | MODE_PARSE_BURST);
  int param, nickpos = 0, banpos = 0, exceptpos = 0;
    ...
    for (param = 3; param < parc; param++)
    {
      ...
      else if (check_modes)
      {
        ...
        for (member = chptr->members; member; member = nmember)
        {
           ...
           */
          if (!(check_modes & MODE_KEY) && (!(check_modes & MODE_INVITEONLY) || IsAnOper(member->user)) && (!(check_modes & MODE_REGONLY) || IsAccount(member->user)) && (!(check_modes & MODE_TLSONLY) || IsTLS(member->user)))
            continue;
  ...
  if (!chptr->creationtime || chptr->creationtime > timestamp)
  {
    ...
    /* mark exceptions for wipeout */
    for (ep = chptr->exceptlist; ep; ep = ep->next)
      ep->flags |= EXCEPT_BURST_WIPEOUT;
  ...
  while (param < parc)
  {
    ...
    case '$': /* parameter contains exceptions */
      if (parse_flags & MODE_PARSE_SET)
      {
        char *exceptlist = parv[param] + 1, *p = 0, *except, *ptr;
        struct Except *newexc;
             ...
             except = ircd_strtok(&p, 0, " "))
        {
          ...
          for (ep_p = &chptr->exceptlist; *ep_p; ep_p = &(*ep_p)->next)
          {
            struct Except *ep2 = *ep_p;
            if (!ircd_strcmp(ep2->exceptstr, except))
            {
              except = 0;                          /* don't add except */
              ep2->flags &= ~EXCEPT_BURST_WIPEOUT; /* not wiping out */
              break;                               /* new except already existed; don't even repropagate */
            }
            else if (!(ep2->flags & EXCEPT_BURST_WIPEOUT) &&
                     !mmatch(ep2->exceptstr, except))
            {
              except = 0; /* don't add except unless wiping out excepts */
              break;      /* new except is encompassed by an existing one; drop */
            }
            else if (!mmatch(except, ep2->exceptstr))
              ep2->flags |= EXCEPT_OVERLAPPED; /* remove overlapping except */
          ...
          if (except)
          { /* add the new except to the end of the list */
            /* Build except buffer */
            if (!exceptpos)
            {
              exceptstr[exceptpos++] = ' ';
            ...
            newexc = make_except(except); /* create new except */
            strcpy(newexc->who, "*");
            newexc->when = TStime();
            newexc->flags |= EXCEPT_BURSTED;
            newexc->next = 0;
            
            if (*ep_p)
              (*ep_p)->next = newexc;
            else
              chptr->exceptlist = newexc;
          }
        }
      }
      param++; /* look at next param */
  ...
  { /* any modes changed? */
    ...
    for (member = chptr->members; member; member = member->next_member)
    {
      if (member->status & CHFL_BURST_JOINED)
      { /* joined during burst */
      ...
      else if (parse_flags & MODE_PARSE_WIPEOUT)
      { /* wipeout old ops */
        if (member->status & CHFL_CHANOP)
        ...
        member->status = (member->status & ~(CHFL_CHANNEL_MANAGER | CHFL_CHANOP | CHFL_VOICE)) | CHFL_DEOPPED;
      }
    }
    ...
    /* Now deal with channel exceptions */
    ep_p = &chptr->exceptlist;
    while (*ep_p)
    {
      ep = *ep_p;

      /* remove exception from channel */
      if (ep->flags & (EXCEPT_OVERLAPPED | EXCEPT_BURST_WIPEOUT))
      {
        char *excdup;
        DupString(excdup, ep->exceptstr);
        modebuf_mode_string(mbuf, MODE_DEL | MODE_EXCEPTION,
                            excdup, 1);
        *ep_p = ep->next; /* clip out of list */
        free_except(ep);
        continue;
      }
      else if (ep->flags & EXCEPT_BURSTED) /* add exception to channel */
        modebuf_mode_string(mbuf, MODE_ADD | MODE_EXCEPTION,
                            ep->exceptstr, 0); /* don't free exceptstr */

      ep->flags &= EXCEPT_IPMASK; /* reset the flag */
      ep_p = &(*ep_p)->next;
    }
  }
