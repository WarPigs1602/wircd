...
#define gliter(list, gl, next)                                                 \
  ...
  for ((gl) = (list); (gl); (gl) = (next))                                     \
    ...
    if ((((next) = (gl)->gl_next) || 1) && /* Then see if it's expired */      \
 ...
 * *host_p. Otherwise, if \a userhost contains '@', assign the earlier part of
 * it to *user_p and the rest to *host_p. Otherwise, assign \a def_user to
 * *user_p and \a userhost to *host_p.
 *
 * @param[in] userhost Input string from user.
 * @param[out] user_p Gets pointer to user (or channel/realname) part of
 * hostmask.
 * @param[out] host_p Gets point to host part of hostmask (may be assigned
 * NULL).
 * @param[in] def_user Default value for user part.
 */
static void canon_userhost(char *userhost, char **user_p, char **host_p,
                           char *def_user) {
  char *tmp;
 ...
 */
static struct Gline *make_gline(char *user, char *host, char *reason,
 ...
 */
static int do_gline(struct Client *cptr, struct Client *sptr,
                    struct Gline *gline) {
  struct Client *acptr;
  ...
  for (fd = HighestFd; fd >= 0; --fd) {
    ...
    if ((acptr = LocalClientArray[fd])) {
      ...
      if (GlineIsRealName(gline)) { /* Realname Gline */
        ...
        if (match(gline->gl_user, (cli_user(acptr))->realusername) != 0)
          continue;
 ...
 */
static int gline_checkmask(char *mask) {
 ...
 * @return Zero.
 */
static int gline_propagate(struct Client *cptr, struct Client *sptr,
                           struct Gline *gline) {
 ...
 * @param[in] flags Bitmask possibly containing the value GLINE_LOCAL, to limit
 * searches to this server.
 * @return Count of matching users.
 */
static int count_users(char *mask, int flags) {
 ...
 * @return Count of matching users.
 */
static int count_realnames(const char *mask) {
  struct Client *acptr;
  int minlen;
  int count;
  char cmask[BUFSIZE];
 ...
 * \li A string starting with $R and followed by a mask to match against their
 * realname.
...
int gline_add(struct Client *cptr, struct Client *sptr, char *userhost,
  ...
  Debug((DEBUG_DEBUG,
         "gline_add(\"%s\", \"%s\", \"%s\", \"%s\", %Tu, %Tu "
         "%Tu, 0x%04x)",
         cli_name(cptr), cli_name(sptr), userhost, reason, expire, lastmod,
         lifetime, flags));
  ...
  if (*userhost == '#' || *userhost == '&') {
    ...
    default:
      ...
      if (IsServer(cptr))
        return protocol_violation(
            sptr,
            "%s has been smoking the sweet leaf and sent me a whacky gline",
            cli_name(sptr));
    ...
    if (sizeof(uhmask) <
      ...
      if ((tmp = count_users(uhmask, flags)) >=
              feature_int(FEAT_GLINEMAXUSERCOUNT) &&
          !(flags & GLINE_OPERFORCE))
        return send_reply(sptr, ERR_TOOMANYUSERS, tmp);
    }
  }
  ...
  /* Inform ops... */
  sendto_opmask_butone(
      0, ircd_strncmp(reason, "AUTO", 4) ? SNO_GLINE : SNO_AUTO,
      "%s adding %s%s %s for %s%s%s, expiring at "
      "%Tu: %s",
      ...
      (flags & (GLINE_BADCHAN | GLINE_REALNAME)) ? "" : host, expire, reason);
  ...
  log_write(LS_GLINE, L_INFO, LOG_NOSNOTICE,
            ...
            flags & GLINE_BADCHAN ? "BADCHAN" : "GLINE", user,
            flags & (GLINE_BADCHAN | GLINE_REALNAME) ? "" : "@",
            flags & (GLINE_BADCHAN | GLINE_REALNAME) ? "" : host, expire,
            reason);
 ...
 * @param[in] flags 0 if the activation should be propagated, GLINE_LOCAL if
 * not.
...
int gline_activate(struct Client *cptr, struct Client *sptr,
  ...
  sendto_opmask_butone(0, SNO_GLINE,
                       "%s activating global %s for %s%s%s, "
                       "expiring at %Tu: %s",
                       (feature_bool(FEAT_HIS_SNOTICES) || IsServer(sptr))
                           ? cli_name(sptr)
                           : cli_name((cli_user(sptr))->server),
                       ...
                       gline->gl_host ? gline->gl_host : "", gline->gl_expire,
                       gline->gl_reason);
...
int gline_deactivate(struct Client *cptr, struct Client *sptr,
  ...
  /* Inform ops and log it */
  sendto_opmask_butone(0, SNO_GLINE,
                       "%s %s %s for %s%s%s, expiring at %Tu: "
                       "%s",
                       (feature_bool(FEAT_HIS_SNOTICES) || IsServer(sptr))
                           ? cli_name(sptr)
                           : cli_name((cli_user(sptr))->server),
 ...
 * @return Zero or CPTR_KILLED, depending on whether \a sptr is suicidal.
 */
int gline_modify(struct Client *cptr, struct Client *sptr, struct Gline *gline,
  ...
  Debug((DEBUG_DEBUG,
         "gline_modify(\"%s\", \"%s\", \"%s%s%s\", %s, \"%s\", "
         "%Tu, %Tu, %Tu, 0x%04x)",
         cli_name(cptr), cli_name(sptr), gline->gl_user,
         gline->gl_host ? "@" : "", gline->gl_host ? gline->gl_host : "",
             ...
             : (action == GLINE_DEACTIVATE
                    ...
                    : (action == GLINE_LOCAL_ACTIVATE
                           ...
                           : (action == GLINE_LOCAL_DEACTIVATE
                                  ...
                                  : (action == GLINE_MODIFY ? "GLINE_MODIFY"
                                                            : "<UNKNOWN>")))),
  ...
  if (flags & GLINE_EXPIRE) {
    ...
    if (pos < BUFSIZE)
      pos += ircd_snprintf(
          0, buf + pos, sizeof(buf) - pos,
          "%s%s changing expiration time to %Tu", pos ? ";" : "",
          pos && !(flags & (GLINE_LIFETIME | GLINE_REASON)) ? " and" : "",
          expire);
  }
  ...
  if (flags & GLINE_LIFETIME) {
    ...
    if (pos < BUFSIZE)
      pos +=
          ircd_snprintf(0, buf + pos, sizeof(buf) - pos,
                        "%s%s extending record lifetime to %Tu", pos ? ";" : "",
                        pos && !(flags & GLINE_REASON) ? " and" : "", lifetime);
  }
  ...
  /* and log the change */
  log_write(
      LS_GLINE, L_INFO, LOG_NOSNOTICE, "%#C modifying global %s for %s%s%s:%s",
      sptr, GlineIsBadChan(gline) ? "BADCHAN" : "GLINE", gline->gl_user,
      gline->gl_host ? "@" : "", gline->gl_host ? gline->gl_host : "", buf);

  /* We'll be simple for this release, but we can update this to change
   * the propagation syntax on future updates
   */
  if (action != GLINE_LOCAL_ACTIVATE && action != GLINE_LOCAL_DEACTIVATE)
    sendcmdto_serv_butone(sptr, CMD_GLINE, cptr, "* %s%s%s%s%s %Tu %Tu %Tu :%s",
 ...
 * @return Zero.
 */
int gline_destroy(struct Client *cptr, struct Client *sptr,
                  struct Gline *gline) {
  assert(gline);
  assert(GlineIsLocal(gline));
  ...
  sendto_opmask_butone(0, SNO_GLINE, "%s removing local %s for %s%s%s",
                       ...
                       gline->gl_user, gline->gl_host ? "@" : "",
                       gline->gl_host ? gline->gl_host : "");
  log_write(LS_GLINE, L_INFO, LOG_NOSNOTICE, "%#C removing local %s for %s%s%s",
            sptr, GlineIsBadChan(gline) ? "BADCHAN" : "GLINE", gline->gl_user,
            gline->gl_host ? "@" : "", gline->gl_host ? gline->gl_host : "");
 ...
 * <dt>GLINE_LASTMOD</dt><dd>Only match G-lines with a last modification
 * time.</dd> <dt>GLINE_EXACT</dt><dd>Require an exact match of G-line
 * mask.</dd> <dt>anything else</dt><dd>Search user G-lines.</dd>
 * </dl>
...
struct Gline *gline_lookup(struct Client *cptr, unsigned int flags) {
  ...
  gliter(GlobalGlineList, gline, sgline) {
    ...
    if (GlineIsRealName(gline)) {
      Debug((DEBUG_DEBUG, "realname gline: '%s' '%s'", gline->gl_user,
             cli_info(cptr)));
...
int gline_list(struct Client *sptr, char *userhost) {
  ...
  if (userhost) {
    ...
    send_reply(sptr, RPL_GLIST, gline->gl_user, gline->gl_host ? "@" : "",
               gline->gl_host ? gline->gl_host : "", gline->gl_expire,
               gline->gl_lastmod, gline->gl_lifetime,
    ...
    gliter(BadChanGlineList, gline, sgline) {
      send_reply(sptr, RPL_GLIST, gline->gl_user, "", "", gline->gl_expire,
                 gline->gl_lastmod, gline->gl_lifetime,
                 GlineIsLocal(gline) ? cli_name(&me) : "*",
                 gline->gl_state == GLOCAL_ACTIVATED
                     ? ">"
 ...
 */
void gline_stats(struct Client *sptr, const struct StatDesc *sd, char *param) {
  struct Gline *gline;
  struct Gline *sgline;
  ...
  gliter(GlobalGlineList, gline, sgline) {
    if (param) {
      if (gline->gl_host)
        ircd_snprintf(NULL, gl_mask, sizeof(gl_mask), "%s@%s", gline->gl_user,
                      gline->gl_host);
      else
    ...
    send_reply(sptr, RPL_STATSGLINE, 'G', gline->gl_user,
               gline->gl_host ? "@" : "", gline->gl_host ? gline->gl_host : "",
               gline->gl_expire, gline->gl_lastmod, gline->gl_lifetime,
               gline->gl_state == GLOCAL_ACTIVATED
                   ? ">"
                   : (gline->gl_state == GLOCAL_DEACTIVATED ? "<" : ""),
               GlineIsRemActive(gline) ? '+' : '-', gline->gl_reason);
 ...
 */
int gline_memory_count(size_t *gl_size) {
  struct Gline *gline;
  unsigned int gl = 0;
...
struct Gline *IsNickGlined(struct Client *cptr, char *nick) {
  struct Gline *gline;
  struct Gline *sgline;
  ...
  for (gline = GlobalGlineList; gline; gline = sgline) {
    ...
    if (GlineIsIpMask(gline)) {
      ...
      if (match(gline->gl_host, cli_user(cptr)->host) != 0 &&
          match(gline->gl_host, cli_sockhost(cptr)) != 0)
        continue;
    }
    return gline;
  }
