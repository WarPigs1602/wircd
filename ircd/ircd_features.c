...
} logTypes[] = {{"FILE", log_set_file, log_get_file},
 ...
 */
static struct LogTypes *feature_log_desc(struct Client *from,
                                         const char *type) {
  int i;
 ...
 */
static int feature_log_set(struct Client *from, const char *const *fields,
                           int count) {
  ...
  if (count < 2) { /* set default facility */
    ...
    if (from)                                    /* send an error */
      ...
      log_write(LS_CONFIG, L_ERROR, 0, "No such logging subsystem \"%s\"",
                fields[0]);
    ...
    if ((*desc->set)(fields[0], count < 3 ? 0 : fields[2])) {
      ...
      if (from) /* send an error */
        ...
        log_write(LS_CONFIG, L_ERROR, 0,
                  "Bad value \"%s\" for log type %s (subsystem %s)", fields[2],
                  desc->type, subsys);
    }
  }
 ...
 */
static int feature_log_reset(struct Client *from, const char *const *fields,
                             int count) {
  struct LogTypes *desc;
  char *subsys;
 ...
 * @param[in] count Number of fields in \a fields.
 */
static void feature_log_get(struct Client *from, const char *const *fields,
                            int count) {
  struct LogTypes *desc;
  char *value, *subsys;
 ...
 */
static void feature_notify_hub(void) {
 ...
 * @return <0 to clear the feature mark, 0 to leave it, >0 to set the feature
 * mark.
 */
typedef int (*feat_set_call)(struct Client *from, const char *const *fields,
                             int count);
 ...
 * @param[in] count Number of fields in \a count.
 */
typedef void (*feat_get_call)(struct Client *from, const char *const *fields,
                              int count);
...
#define F_N(type, flags, set, reset, get, notify, unmark, mark, report)        \
  {FEAT_##type, #type,    FEAT_NONE | (flags),                                 \
   0,           0,        0,                                                   \
   0,           (set),    (reset),                                             \
   (get),       (notify), (unmark),                                            \
   (mark),      (report)}
...
#define F_S(type, flags, v_str, notify)                                        \
  {FEAT_##type, #type, FEAT_STR | (flags), 0, 0, 0, (v_str), 0,                \
   0,           0,     (notify),           0, 0, 0}
...
static struct FeatureDesc {
  ...
  feat_report_call report; /**< report feature values */
    ...
    F_S(HIS_SERVERINFO, 0, "The Undernet Underworld",
 ...
 */
static struct FeatureDesc *feature_desc(struct Client *from,
                                        const char *feature) {
  int i;
  ...
  if (count < 1) {
    ...
    case FEAT_STR: /* it's a string value */
      ...
      if (t_str == feat->def_str ||
          (t_str && feat->def_str &&
           !(feat->flags & FEAT_CASE
                 ? strcmp(t_str, feat->def_str)
                 : ircd_strcmp(t_str,
                               feat->def_str)))) { /* resetting to default */
...
int feature_get(struct Client *from, const char *const *fields, int count) {
  ...
  if (count < 1) /* check parameters */
    ...
    case FEAT_INT: /* integer, report integer value */
      send_reply(from, SND_EXPLICIT | RPL_FEATURE, ":Integer value of %s: %d",
                 feat->type, feat->v_int);
      break;

    case FEAT_BOOL: /* boolean, report boolean value */
      send_reply(from, SND_EXPLICIT | RPL_FEATURE, ":Boolean value of %s: %s",
                 feat->type, feat->v_int ? "TRUE" : "FALSE");
      break;

    case FEAT_STR:     /* string, report string value */
      if (feat->v_str) /* deal with null case */
        send_reply(from, SND_EXPLICIT | RPL_FEATURE, ":String value of %s: %s",
                   feat->type, feat->v_str);
      else
        send_reply(from, SND_EXPLICIT | RPL_FEATURE,
 ...
 */
void feature_report(struct Client *to, const struct StatDesc *sd, char *param) {
  char changed;
  int report;
  int i;
  ...
  for (i = 0; features[i].type; i++) {
    ...
    case FEAT_INT: /* Report an F-line with integer values */
      if (report)  /* it's been changed */
        send_reply(to, SND_EXPLICIT | RPL_STATSFLINE, "%c %s %d", changed,
                   features[i].type, features[i].v_int);
      break;

    case FEAT_BOOL: /* Report an F-line with boolean values */
      if (report)   /* it's been changed */
        send_reply(to, SND_EXPLICIT | RPL_STATSFLINE, "%c %s %s", changed,
    ...
    case FEAT_STR:  /* Report an F-line with string values */
      if (report) { /* it's been changed */
        if (features[i].v_str)
          ...
          send_reply(to, SND_EXPLICIT | RPL_STATSFLINE, "%c %s", changed,
                     features[i].type);
      }
      break;
    }
