...
static inline unsigned int downsample(unsigned char *i) {
  ...
  return (((unsigned int)r[0] << 24) + ((unsigned int)r[1] << 16) +
...
static char *hidehost_ipv6(char *host) {
  ...
  /* BETA... */
  ircd_snprintf(0, buf, HOSTLEN, "%s:%s:%s", KEY3, host, KEY1);
  DoMD5(res, (unsigned char *)buf, strlen(buf));
  ...
  /* GAMMA... */
  ircd_snprintf(0, buf, HOSTLEN, "%s:%s:%s", KEY2, host, KEY1);
  DoMD5(res, (unsigned char *)buf, strlen(buf));
  strcpy((char *)res + 16, KEY3);
  n = strlen((char *)res + 16) + 16;
  DoMD5(res2, res, n);
  ...
  /* lower case X? */
  ircd_snprintf(0, result, HOSTLEN, "%x.%x.%x.%x.ip", alpha, beta, gamma,
                delta);
  return result;
}
...
static char *hidehost_normalhost(char *host) {
  ...
  if (*p) {
    ...
    ircd_snprintf(0, result, HOSTLEN, "%s-%x.",
                  feature_str(FEAT_HOST_HIDING_PREFIX), alpha);
...
char *cloak_host(char *host)
{
    ...
    if (strchr(host, ':')) {
        ...
        char ipv6_string[45];
        inet_pton(AF_INET6, host, &ipv6_addr);
        if (inet_ntop(AF_INET6, &ipv6_addr, ipv6_string, sizeof(ipv6_string)) != NULL) {
            return hidehost_ipv6(ipv6_string);
        }
    }
    unsigned int a, b, c, d;
