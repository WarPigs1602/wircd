...
/** Freelist for struct Except*'s */
static struct Except *free_excepts;
/** Number of except structures allocated. */
static size_t excepts_alloc;
/** Number of except structures in use. */
static size_t excepts_inuse;
...
/** Set the mask for an exception, checking for IP masks.
 * @param[in,out] except Exception structure to modify.
 * @param[in] exceptstr Mask to except.
 */
static void
set_except_mask(struct Except *except, const char *exceptstr)
{
  char *sep;
  assert(exceptstr != NULL);
  ircd_strncpy(except->exceptstr, exceptstr, sizeof(except->exceptstr) - 1);
  sep = strrchr(exceptstr, '@');
  if (sep)
  {
    except->nu_len = sep - exceptstr;
    if (ipmask_parse(sep + 1, &except->address, &except->addrbits))
      except->flags |= EXCEPT_IPMASK;
  }
}

/** Allocate a new Except structure.
 * @param[in] exceptstr Exception mask to use.
 * @return Newly allocated exception.
 */
struct Except *
make_except(const char *exceptstr)
{
  struct Except *except;
  if (free_excepts)
  {
    except = free_excepts;
    free_excepts = free_excepts->next;
  }
  else if (!(except = MyMalloc(sizeof(*except))))
    return NULL;
  else
    excepts_alloc++;
  excepts_inuse++;
  memset(except, 0, sizeof(*except));
  set_except_mask(except, exceptstr);
  return except;
}

/** Deallocate an except structure.
 * @param[in] except Exception to deallocate.
 */
void free_except(struct Except *except)
{
  except->next = free_excepts;
  free_excepts = except;
  excepts_inuse--;
}

/** Report except usage to \a cptr.
 * @param[in] cptr Client requesting information.
 */
void excepts_send_meminfo(struct Client *cptr)
{
  struct Except *except;
  size_t num_free;
  for (num_free = 0, except = free_excepts; except; except = except->next)
    num_free++;
  send_reply(cptr, SND_EXPLICIT | RPL_STATSDEBUG, ":Excepts: inuse %zu(%zu) free %zu alloc %zu",
             excepts_inuse, excepts_inuse * sizeof(*except), num_free, excepts_alloc);
}
...
int sub1_from_channel(struct Channel *chptr)
{
  ...
  if (!chptr->mode.apass[0]) /* If no Apass, reset all modes. */
  {
    ...
    *chptr->mode.key = '\0';
    ...
    for (elink = chptr->exceptlist; elink; elink = enext)
    {
      enext = elink->next;
      free_except(elink);
    }
...
/** Searches for an exception from an exception list that matches a user.
 * @param[in] cptr The client to test.
 * @param[in] exceptlist The list of exceptions to test.
 * @return Pointer to a matching exception, or NULL if none exit.
 * Note that this function is not used for channel access checks, but only for
 * checking if a user is excepted from a ban.  Therefore, it only checks the hostmask and not
 * the nick!user portion of the exception.
 */
struct Except *find_except(struct Client *cptr, struct Except *exceptlist)
{
  char tmphost[HOSTLEN + 1];
  char iphost[SOCKIPLEN + 1];
  char *hostmask;
  ...
  /* Walk through exception list. */
  for (found = NULL; exceptlist; exceptlist = exceptlist->next)
  {
    int res;
    /* Compare host portion of exception. */
    hostmask = exceptlist->exceptstr;
    if (!((exceptlist->flags & EXCEPT_IPMASK) && ipmask_check(&cli_ip(cptr), &exceptlist->address, exceptlist->addrbits)) && match(hostmask, cli_user(cptr)->host) && match(hostmask, iphost) && !(sr && !match(hostmask, sr)))
      continue;
    /* Otherwise, remember this exception but keep searching for a ban. */
    found = exceptlist;
  }
  return found;
}
 ...
 * This function will check the except cache if applicable, otherwise will
 * do the comparisons and cache the result.
 * Note that this function is not used for channel access checks, but only for checking if a user is excepted from a ban.  Therefore, it only checks the hostmask and not the nick!user portion of the exception.
 * @param[in] member The Membership to test for excepted-ness.
 * @return Non-zero
 * if the member is excepted, zero if not.
 */
static int is_excepted(struct Membership *member)
{
  ...
  if (find_except(member->user, member->channel->exceptlist))
  {
    SetExcepted(member);
    return 1;
  }
...
int client_can_send_to_channel(struct Client *cptr, struct Channel *chptr, int reveal)
{
  ...
  return !find_ban(cptr, chptr->banlist) && !find_except(cptr, chptr->exceptlist);
}
...
const char *find_no_nickchange_channel(struct Client *cptr)
{
  if (MyUser(cptr))
  {
         ...
         member = member->next_channel)
    {
      if (IsVoicedOrOpped(member))
        continue;
      if ((member->channel->mode.mode & MODE_MODERATED) || (member->channel->mode.mode & (MODE_MODERATENOREG | MODE_REGONLY) && !IsAccount(cptr)) || (is_banned(member) && !is_excepted(member)))
  ...
  *modebuf = *parabuf = '\0';
  ...
  for (first = 1; full; first = 0) /* Loop for multiple messages */
  {
    ...
    if (!full)
    {
      ...
      if (!full && emitted_bans && ep2)
      {
        full = 1;
      }
      else if (!full)
      {      
      /* Attach all exceptions, space separated " :$except except ..." */
      for (first = 2, ep2 = chptr->exceptlist; ep2; ep2 = ep2->next)
      {
        len = strlen(ep2->exceptstr);
        if (msgq_bufleft(mb) < len + 1 + first)
 ...
 *
 * @param cptr	Client to send the exception list to.
...
static void send_exception_list(struct Client *cptr, struct Channel *chptr)
{
  ...
  for (lp = chptr->exceptlist; lp; lp = lp->next)
    send_reply(cptr, RPL_EXCEPTLIST, chptr->chname, lp->exceptstr,
               lp->who, lp->when);
...
modebuf_flush_int(struct ModeBuf *mbuf, int all)
{
                       ...
                       MODEBUF_DEST_LOG))
  {
    ...
    for (i = 0; i < mbuf->mb_count; i++)
    {
      ...
      else if (MB_TYPE(mbuf, i) & MODE_EXCEPTION)
...
void modebuf_mode(struct ModeBuf *mbuf, unsigned int mode)
{
  ...
  mode &= (MODE_ADD | MODE_DEL | MODE_PRIVATE | MODE_SECRET | MODE_MODERATED |
           ...
           MODE_DELJOINS | MODE_WASDELJOINS | MODE_REGISTERED | MODE_NOKNOCK);
...
/** Simple function to invalidate a channel's exception cache.
 *
 * This function marks all members of the channel as being neither
 * excepted nor excepted.
 *
 * @param chan	The channel to operate on.
 */
void mode_exception_invalidate(struct Channel *chan)
{
  struct Membership *member;
...
struct ParseState
{
  ...
  struct Except exceptlist[MAXPARA];
  struct
  {
    unsigned int flag;
...
/** Compare one exception's extent to another.
 * This works very similarly to bmatch() but it knows that exceptions
 * are only supersets of other exceptions, and that they are never
 * supersets of bans.  If both exceptions are CIDR-based, compare their
 * address bits; otherwise, use mmatch().
 * @param[in] old_exc One exception.
 * @param[in] new_exc Another exception.
 * @return Zero if \a old_exc is a superset of \a new_exc, non-zero otherwise.
 */
static int
ematch(struct Except *old_exc, struct Except *new_exc)
{
  int res;
  assert(old_exc != NULL);
  assert(new_exc != NULL);
  /* An exception is never treated as a superset of a ban. */
  if ((old_exc->flags & EXCEPT) == 0 && (new_exc->flags & EXCEPT))
    return 1;
  /* If either is not an address mask, match the text masks. */
  if ((old_exc->flags & new_exc->flags & EXCEPT_IPMASK) == 0)
    return mmatch(old_exc->exceptstr, new_exc->exceptstr);
  /* If the old exception has a longer prefix than new, it cannot be a superset. */
  if (old_exc->addrbits > new_exc->addrbits)
    return 1;
  /* Compare the masks before the hostname part.  */
  old_exc->exceptstr[old_exc->nu_len] = new_exc->exceptstr[new_exc->nu_len] = '\0';
  res = mmatch(old_exc->exceptstr, new_exc->exceptstr);
  old_exc->exceptstr[old_exc->nu_len] = new_exc->exceptstr[new_exc->nu_len] = '@';
  if (res)
    return res;
  /* If the old exception's mask mismatches, cannot be a superset. */
  if (!ipmask_check(&new_exc->address, &old_exc->address, old_exc->addrbits))
    return 1;
  /* Otherwise it depends on whether the old exception's text is a superset
   * of the new. */
  return mmatch(old_exc->exceptstr, new_exc->exceptstr);
}
...
/** Add an exception from an exception list and mark exceptions that should be removed
 * because they overlap.
 *
 * There are three invariants for an exception list.  First, no ban may be
 * more specific than another ban.  Second, no exception may be more
 * specific than another exception.  Finally, no ban may be more
 * specific than any exception.
 *
 * @param[in,out] exceptlist Pointer to head of list.
 * @param[in] newexc Exception (or ban) to add (or remove).
 * @param[in] do_free If non-zero, free \a newexc on failure.
 * @return Zero if \a newexc could be applied, non-zero if not.
 */
int apply_except(struct Except **exceptlist, struct Except *newexc, int do_free)
{
  struct Except *exc;

  assert(newexc->flags & (EXCEPT | EXCEPT_DEL));
  if (newexc->flags & EXCEPT)
  {
    /* If a less specific *active* entry is found, fail.  */
    for (exc = *exceptlist; exc; exc = exc->next)
    {
      if (!ematch(exc, newexc) && !(exc->flags & EXCEPT_DEL))
      {
        if (do_free)
          free_except(newexc);
        return 1;
    ...
    /* Mark more specific entries and add this one to the end of the list. */
    while ((exc = *exceptlist) != NULL)
    {
      if (!ematch(newexc, exc))
      {
        exc->flags |= EXCEPT_OVERLAPPED | EXCEPT_DEL;
      }
      exceptlist = &exc->next;
    }
    *exceptlist = newexc;
  ...
  else if (newexc->flags & EXCEPT_DEL)
  {
    size_t remove_count = 0;
    /* Mark more specific entries. */
    for (exc = *exceptlist; exc; exc = exc->next)
...
/*
 * Helper function to convert exceptions
 */
static void
mode_parse_except(struct ParseState *state, int *flag_p)
{
  char *t_str, *s;
  struct Except *exc, *newexc;

  if (state->parc <= 0)
  { /* Not enough args, send exception list */
  ...
  /* Clear all EXCEPT/DEL/OVERLAPPED flags from exception list. */
  if (!(state->done & DONE_EXCEPTCLEAN))
  {
    for (exc = state->chptr->exceptlist; exc; exc = exc->next)
      exc->flags &= ~(EXCEPT | EXCEPT_DEL | EXCEPT_OVERLAPPED);
    state->done |= DONE_EXCEPTCLEAN;
  }

  /* remember the exception for the moment... */
  if (state->numexcepts >= MAXPARA)
    return;
  newexc = state->exceptlist + (state->numexcepts++);
  newexc->next = 0;
  newexc->flags = ((state->dir == MODE_ADD) ? EXCEPT : EXCEPT_DEL) | (*flag_p == MODE_EXCEPTION ? 0 : EXCEPT);
  set_except_mask(newexc, collapse(pretty_mask(t_str)));
  ircd_strncpy(newexc->who, IsUser(state->sptr) ? cli_name(state->sptr) : "*", NICKLEN);
  newexc->when = TStime();
  apply_except(&state->chptr->exceptlist, newexc, 0);
}
...
/*
 * This is the bottom half of the exception processor
 */
static void
mode_process_excepts(struct ParseState *state)
{
  struct Except *exc, *newexc, *prevexc, *nextexc;
  int count = 0;
  int len = 0;
  int exc_len;
  int changed = 0;

  for (prevexc = 0, exc = state->chptr->exceptlist; exc; exc = nextexc)
  {
    count++;
    exc_len = strlen(exc->exceptstr);
    len += exc_len;
    nextexc = exc->next;

    if ((exc->flags & (EXCEPT_DEL | EXCEPT)) == (EXCEPT_DEL | EXCEPT))
    {
      if (prevexc)
        prevexc->next = 0; /* Break the list; exc isn't a real exception */
      else
    ...
    else if (exc->flags & EXCEPT_DEL)
    { /* Deleted an exception? */
      char *excexc;
      DupString(excexc, exc->exceptstr);
      modebuf_mode_string(state->mbuf, MODE_DEL | MODE_EXCEPTION, excexc, 1);

      if (state->flags & MODE_PARSE_SET)
      {              /* Ok, make it take effect */
        if (prevexc) /* clip it out of the list... */
          prevexc->next = exc->next;
        else
          state->chptr->exceptlist = exc->next;
        ...
        changed++;
        continue; /* next exception; keep prevexc like it is */
      }
      else
        exc->flags &= EXCEPT_IPMASK; /* unset other flags */
    }
    else if (exc->flags & EXCEPT)
    { /* adding an exception? */
      if (prevexc)
        prevexc->next = 0; /* Break the list; exc isn't a real exception */
      else
      ...
      /* If we're supposed to ignore it, do so. */
      if (exc->flags & EXCEPT_OVERLAPPED && !(state->flags & MODE_PARSE_BOUNCE))
      {
        count--;
        len -= exc_len;
      }
      else
      {
        ...
        else
        {
          char *excexc;
          DupString(excexc, exc->exceptstr);
          modebuf_mode_string(state->mbuf, MODE_ADD | MODE_EXCEPTION, excexc, 1);

          if (state->flags & MODE_PARSE_SET)
          { /* create a new exception */
            newexc = make_except(exc->exceptstr);
            if (newexc)
            {
              strcpy(newexc->who, exc->who);
              newexc->when = exc->when;
              newexc->flags = exc->flags & EXCEPT_IPMASK;

              newexc->next = state->chptr->exceptlist;
              state->chptr->exceptlist = newexc;
              changed++;
            }
          }
        }
  ...
  if (changed) /* if we changed the except list, we must invalidate the excepts */
    mode_exception_invalidate(state->chptr);
}
               ...
               struct Membership *member)
{
  ...
  if (state.done & DONE_EXCEPTCLEAN) /* process exceptions */
