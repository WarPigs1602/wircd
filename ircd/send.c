...
static void dead_link(struct Client *to, char *notice) {
  ...
  if (!IsUser(to) && !IsUnknown(to) && !HasFlag(to, FLAG_CLOSING))
    sendto_opmask_butone(0, SNO_OLDSNO, "%s for %s", cli_info(to),
                         cli_name(to));
    ...
    if ((len = deliver_it(to, &(cli_sendQ(to))))) {
      ...
      if (IsDead(to)) {
        ...
        sprintf(tmp, "Write error: %s",
                (strerror(cli_error(to))) ? (strerror(cli_error(to)))
                                          : "Unknown error");
        dead_link(to, tmp);
      }
      return;
  ...
  if (MsgQLength(&(cli_sendQ(to))) > get_sendq(to)) {
    if (IsServer(to))
      sendto_opmask_butone(0, SNO_OLDSNO,
                           "Max SendQ limit exceeded for %C: "
                           "%zu > %zu",
...
static int match_it(struct Client *from, struct Client *one, const char *mask,
  ...
  case MATCH_HOST:
    return (match(mask, cli_user(one)->host) == 0 ||
            ((HasHiddenHost(one) || HasSetHost(one)) &&
             match(mask, cli_user(one)->realhost) == 0));
  case MATCH_SERVER:
  default:
...
void sendcmdto_flagarray_serv_butone(struct Client *from, const char *cmd,
  ...
  for (lp = cli_serv(&me)->down; lp; lp = lp->next) {
    ...
    for (i = 0; i < requiresize; i++) {
      if ((require[i] < FLAG_LAST_FLAG) &&
          !HasFlag(lp->value.cptr, require[i])) {
        skip = 1;
        break;
      }
    }
 ...
 */
static void bump_sentalong(struct Client *one) {
  if (!++sentalong_marker) {
    int ii;
 ...
 * @param[in] pattern Format string for command arguments.
 */
void sendcmdto_capflag_common_channels_butone(struct Client *from,
...
/** Send a (prefixed) command to all local users on a channel matching or not
 * matching a capability flag..
 * @param[in] from Client originating the command.
 * @param[in] cmd Long name of command.
 * @param[in] tok Short name of command (ignored).
 * @param[in] to Destination channel.
 * @param[in] one Client direction to skip (or NULL).
 * @param[in] skip Bitmask of SKIP_DEAF, SKIP_NONOPS, SKIP_NONVOICES indicating
 * which clients to skip.
 * @param[in] require Only send to clients with this Flag bit set.
 * @param[in] forbid Do not send to clients with this Flag bit set.
 * @param[in] pattern Format string for command arguments.
 */
void sendcmdto_capflag_channel_butserv_butone(
    struct Client *from, const char *cmd, const char *tok, struct Channel *to,
    struct Client *one, unsigned int skip, capset_t require, capset_t forbid,
    const char *pattern, ...) {
  struct VarData vd;
  struct MsgBuf *mb;
  struct Membership *member;
...
void sendjointo_channel_butserv(struct Client *from, struct Channel *chptr,
  ...
  sendcmdto_capflag_channel_butserv_butone(
      from, CMD_JOIN, chptr, NULL, 0, require | CAP_EXTJOIN, forbid,
      "%H %s :%s", chptr, IsAccount(from) ? cli_account(from) : "*",
      cli_info(from));
  sendcmdto_capflag_channel_butserv_butone(from, CMD_JOIN, chptr, NULL, 0,
                                           require, forbid | CAP_EXTJOIN, "%H",
                                           chptr);
}
 ...
 */
void sendjointo_one(struct Client *from, struct Channel *chptr,
                    struct Client *one) {
 ...
 * @param[in] to Destination channel.
 * @param[in] one Client direction to skip (or NULL).
 * @param[in] skip Bitmask of SKIP_NONOPS and SKIP_NONVOICES indicating which
 * clients to skip.
 * @param[in] pattern Format string for command arguments.
 */
...
void sendcmdto_channel_butone(struct Client *from, const char *cmd,
  ...
  va_start(vd.vd_args, pattern);
  user_mb = msgq_make(
      0, skip & (SKIP_NONOPS | SKIP_NONVOICES) ? "%:#C %s @%v" : "%:#C %s %v",
  ...
  for (member = to->members; member; member = member->next_member) {
    ...
    if (IsZombie(member) || (skip & SKIP_DEAF && IsDeaf(member->user)) ||
        (skip & SKIP_NONOPS && !IsChanOp(member)) ||
        (skip & SKIP_NONVOICES && !IsChanOp(member) && !HasVoice(member)) ||
...
void sendwallto_group_butone(struct Client *from, int type, struct Client *one,
  ...
  for (i = 0; i <= HighestFd; i++) {
    if (!(cptr = LocalClientArray[i]) || (cli_fd(cli_from(cptr)) < 0) ||
...
void sendcmdto_match_butone(struct Client *from, const char *cmd,
                            const char *tok, const char *to, struct Client *one,
                            unsigned int who, const char *pattern, ...) {
  struct VarData vd;
  struct Client *cptr;
  struct MsgBuf *user_mb;
  struct MsgBuf *serv_mb;
...
void sendcmdto_tagmsg_butone(struct Client *from, struct Channel *to,
  ...
  /* Build buffer to send to servers */
  va_start(vd.vd_args, pattern);
  serv_mb = msgq_make(&me, "%v %:#C TAGMSG %H", &vd, from, to);
  va_end(vd.vd_args);

  bump_sentalong(one);
  if (to) {
    for (member = to->members; member; member = member->next_member) {
      if (IsZombie(member) || cli_fd(cli_from(member->user)) < 0 ||
          cli_sentalong(member->user) == sentalong_marker)
        continue;
      cli_sentalong(member->user) = sentalong_marker;
...
/** Send tagmsg to user that matching the message-tags capability flag
 * @param[in] from Client originating the command.
 * @param[in] to Destination channel.
 * @param[in] one Client direction to skip (or NULL).
 * @param[in] pattern Format string for command arguments.
 */
void sendcmdto_tagmsg_priv_butone(struct Client *from,
                                          struct Client *to, struct Client *one,
                                          const char *pattern, ...) {
  struct VarData vd;
  vd.vd_format = pattern;
  if (MyConnect(to)) {
    ...
    struct DLink *lp;
    va_start(vd.vd_args, pattern);
    serv_mb = msgq_make(&me, "%v %:#C TAGMSG %s", &vd, from, cli_name(to));
    va_end(vd.vd_args);
    for (lp = cli_serv(&me)->down; lp; lp = lp->next) {
      if (one && lp->value.cptr == cli_from(one))
        continue;
      send_buffer(lp->value.cptr, serv_mb, 0);
    }
    msgq_clean(serv_mb);
  }
}