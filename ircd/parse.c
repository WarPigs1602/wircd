...
struct Message msgtab[] = {
     ...
     * - ASUKA
     * --------------------------------------------------------------------- Add
     * the command for CHECK. This was adapted from Lain for use in Asuka.
 ...
 */
void add_msg_element(struct MessageTree *mtree_p, struct Message *msg_p,
                     char *cmd) {
...
int parse_client(struct Client *cptr, char *buffer, char *bufend) {
  ...
  char *tags = NULL; /* IRCv3 message tags prefix if present */
  ...
  /* IRCv3 tags prefix */
  if (*ch == '@') {
    tags = ch; /* remember start of tag block (including '@') */
    while (*ch && *ch != ' ')
      ++ch;
    if (*ch == ' ') {
      *ch++ = '\0'; /* terminate tag block and advance to next token */
      while (*ch == ' ')
        ++ch;
    }
  }
  ...
  if ((mptr = msg_tree_parse(ch, &msg_tree)) == NULL) {
    ...
    if (buffer[0] != '\0') {
      ...
      Debug((DEBUG_ERROR, "Unknown (%s) from %s", ch,
  ...
  if (!feature_bool(FEAT_IDLE_FROM_MSG) && IsUser(cptr) && handler != m_ping &&
      handler != m_ignore)
...
int parse_server(struct Client *cptr, char *buffer, char *bufend) {
  ...
  /* IRCv3 tags may precede the server/user prefix */
  while (*ch == ' ')
    ++ch;
  if (*ch == '@') {
    tags = ch;
  ...
  if (*ch == ':') {
    ...
    *ch++ = '\0';
     ...
     */
    if (from == NULL) {
      Debug((DEBUG_NOTICE, "Unknown prefix (%s)(%s) from (%s)", para[0], buffer,
             cli_name(cptr)));
      ++ServerStats->is_unpf;
      while (*ch == ' ')
        ch++;
  ...
  if (len == 3 && IsDigit(*ch)) {
    ...
    if ((numeric < 1) || (numeric > 999)) {
      Debug((DEBUG_NOTICE, "Bad numeric: Message (%s) coming from (%s)", buffer,
             cli_name(cptr)));
      return -1;
    }
